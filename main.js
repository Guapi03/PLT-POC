...
import { setupModelEditor } from './model-editor.js';
import { applyMaterialAppearance } from './appearance.js';

const $=s=>document.querySelector(s);
const state={loaded:false,busy:false,lift:0,selected:null,view:'glass',hideBottle:false,autoRotate:false,inAR:false,modelId:null,profile:'bottle',settings:null};
...
const parts={},partInfo={},originals=new Map(),meshById=new Map();let active=null,library,ar,editor,meshDescriptors=[];
...
function applyAppearance(arMode=false){
 for(const [obj,saved] of originals){
  const bases=[].concat(saved.material),group=state.settings?.groups.find(g=>g.id===saved.part);
  [].concat(obj.material).forEach((m,i)=>applyMaterialAppearance(m,bases[i],{
   preset:group?.preset||'auto',glassOpacity:state.settings?.glassOpacity??.18,
   view:state.view,arMode,selected:state.selected===saved.part
  }));
 }
}
function selectPart(part){
 ...
 $('#detail-title').textContent=part?partInfo[part].name:'选择部件，查看模型。';$('#detail-copy').textContent=part?(partInfo[part].description||`包含 ${parts[part].length} 个网格，点击「编辑模型」可改名或分组。`):'点击模型或上方列表，定位一个部件。';applyAppearance(state.inAR);return getState();
}
function setLift(value,{fit=true}={}){
 const mm=Number(value),config=state.settings?.assembly;
 ...
 state.lift=mm;assembly.position.set(0,0,0);if(config?.enabled)assembly.position[config.axis]=mm/1000*config.direction;
 $('#lift').value=String(mm);$('#lift-value').textContent=Number(mm.toFixed(1))+' mm';$('#assembly-toggle').textContent=mm>0?'复原组装 ↓':'展开部件 ↑';product.updateMatrixWorld(true);if(fit)fitView();ar?.invalidate();return getState();
}
function setHidden(hidden){
 ...
 for(const [id,meshes] of Object.entries(parts))for(const obj of meshes)obj.visible=!(hidden&&config.hiddenGroupIds.includes(id));
 $('#hide-bottle').checked=!!hidden;ar?.invalidate();return getState();
}
function setView(view){if(!['glass','original','solid'].includes(view))throw new Error('显示方式无效。');state.view=view;document.querySelectorAll('[data-view]').forEach(b=>{b.classList.toggle('active',b.dataset.view===view);b.setAttribute('aria-pressed',String(b.dataset.view===view));});applyAppearance(state.inAR);return getState();}
function getState(){return {...state,settings:state.settings?structuredClone(state.settings):null,modelName:active?.name,parts:Object.entries(parts).map(([id,meshes])=>({id,name:partInfo[id].name,meshCount:meshes.length,meshIds:partInfo[id].meshIds})),materials:[...originals.keys()].map(obj=>({meshId:obj.userData.meshId,part:obj.userData.part,opacity:[].concat(obj.material).map(m=>m.opacity),transmission:[].concat(obj.material).map(m=>m.transmission||0)}))};}
...
function drawParts(){
 ...
 Object.keys(parts).forEach((id,i)=>{const b=document.createElement('button');b.className='part-button';b.dataset.part=id;b.setAttribute('aria-pressed','false');const number=document.createElement('span');number.className='part-number';number.textContent=String(i+1).padStart(2,'0');const label=document.createElement('span');label.textContent=partInfo[id].name;b.append(number,label);b.onclick=()=>selectPart(state.selected===id?null:id);$('#part-list').append(b);});$('#part-count').textContent=Object.keys(parts).length+' 个部件';
}
function applySettings(raw){
 state.settings=normalizeModelSettings(raw,meshDescriptors,{profile:state.profile});
 // Static mesh matrices are already in product space; regrouping does not alter them.
 assembly.position.set(0,0,0);for(const obj of meshById.values()){product.add(obj);obj.visible=true;}
 for(const id of Object.keys(parts))delete parts[id];for(const id of Object.keys(partInfo))delete partInfo[id];
 for(const group of state.settings.groups){
  parts[group.id]=group.meshIds.map(id=>meshById.get(id)).filter(Boolean);partInfo[group.id]=group;
  for(const obj of parts[group.id]){obj.userData.part=group.id;originals.get(obj).part=group.id;}
 }
 const config=state.settings.assembly;
 if(config.enabled)for(const id of config.movingGroupIds)for(const obj of parts[id]||[])assembly.add(obj);
 $('#assembly-section').hidden=!config.enabled;$('#lift').max=String(config.distanceMm);$('#lift-max').textContent=config.distanceMm+' mm';
 $('#hide-parts-row').hidden=!config.hiddenGroupIds.length;
 $('#hide-parts-label').textContent='隐藏：'+config.hiddenGroupIds.map(id=>partInfo[id]?.name).filter(Boolean).join('、');
 drawParts();setLift(0,{fit:false});setHidden(false);selectPart(null);fitView();ar?.invalidate();
}
async function activate(entry,gltf){
 if(state.inAR)throw new Error('请先退出 AR。');
 state.loaded=false;ar?.invalidate();
 // Dispose display clones, then the retained source materials and their textures.
 for(const [obj,saved] of originals){[].concat(obj.material).forEach(m=>m.dispose());obj.material=saved.material;}
 disposeModel(product);originals.clear();meshById.clear();product.clear();assembly.clear();assembly.position.set(0,0,0);product.add(assembly);product.position.set(0,0,0);product.quaternion.identity();product.scale.set(1,1,1);
 ...
 const loaded=[];model.traverse(obj=>{if(obj.isMesh)loaded.push({obj,matrix:obj.matrixWorld.clone()});});meshDescriptors=[];
 // Preserve world matrices exactly, including scaled/rotated parent transforms.
 loaded.forEach(({obj,matrix},index)=>{
  const id='mesh-'+index;meshDescriptors.push({id,name:obj.name||'部件 '+(index+1)});meshById.set(id,obj);obj.userData.meshId=id;
  const base=obj.material,cloned=[].concat(base).map(m=>m.clone());obj.material=cloned.length===1?cloned[0]:cloned;originals.set(obj,{material:base,part:null});
  product.add(obj);obj.matrixAutoUpdate=false;obj.matrix.copy(matrix);obj.matrixWorldNeedsUpdate=true;
 ...
 $('#model-title').textContent=entry.name;$('#model-subtitle').textContent=entry.description||'独立配件模型';$('#model-note').textContent=state.profile==='bottle'?'照片估算尺寸 · 容积未校准':'按 GLB 导出尺寸展示';
 $('#scale-note').textContent=`MODEL HEIGHT ≈ ${(size.y*1000).toLocaleString('en',{maximumFractionDigits:1})} mm`;
 state.loaded=true;applySettings(entry.settings);reset();$('#loading').hidden=true;
}
$('#lift').addEventListener('input',e=>setLift(e.target.value));$('#assembly-toggle').addEventListener('click',()=>setLift(state.lift>0?0:state.settings.assembly.distanceMm));$('#hide-bottle').addEventListener('change',e=>setHidden(e.target.checked));$('#auto-rotate').addEventListener('change',e=>{state.autoRotate=e.target.checked;controls.autoRotate=state.autoRotate;});$('#reset-camera').addEventListener('click',()=>fitView(true));$('#retry').addEventListener('click',()=>location.reload());
...
editor=setupModelEditor({
 ...
 onPreview:settings=>{applySettings(settings);setView('glass');},
 onSave:async patch=>{
  const settings=normalizeModelSettings(patch.settings,meshDescriptors,{profile:state.profile});
  active=await library.updateEntry(active.id,{...patch,settings});
  $('#model-title').textContent=active.name;$('#model-subtitle').textContent=active.description||'独立配件模型';applySettings(active.settings);setView('glass');
  $('#model-note').textContent='设置已保存在此浏览器 · 导出网站包可发布';
 }
});
...
async function init(){
 try{library=await setupLibrary({activate,getActive:()=>active,isBusy:()=>state.inAR||state.arStarting,onBusyChange:busy=>{state.busy=busy;$('#open-ar').disabled=busy||!state.loaded;$('#edit-model').disabled=busy||!state.loaded;}});$('#open-ar').disabled=false;$('#edit-model').disabled=false;registerTools();}
...
function registerTools(){
 ...
 const tools=[
  ...
  {name:'configure_glass_model',description:'Change preview controls. Assembly controls require the model to have assembly enabled in its editor.',inputSchema:{type:'object',properties:{lift_mm:{type:'number',minimum:0,maximum:1000},hide_bottle:{type:'boolean'},part:{type:['string','null']},view:{type:'string',enum:['glass','original','solid']}},additionalProperties:false},annotations:{readOnlyHint:false},execute(input){
   if(state.busy||state.inAR||editor.isOpen())throw new Error('模型正忙，请稍后操作。');if(!input||typeof input!=='object'||Array.isArray(input))throw new Error('Expected object');for(const k of Object.keys(input))if(!['lift_mm','hide_bottle','part','view'].includes(k))throw new Error('Unknown setting');
   ...
   if('hide_bottle'in input&&(typeof input.hide_bottle!=='boolean'||!state.settings.assembly.enabled||!state.settings.assembly.hiddenGroupIds.length))throw new Error('Invalid hide_bottle');if('part'in input&&input.part!==null&&!Object.hasOwn(parts,input.part))throw new Error('Invalid part');if('view'in input&&!['glass','original','solid'].includes(input.view))throw new Error('Invalid view');
